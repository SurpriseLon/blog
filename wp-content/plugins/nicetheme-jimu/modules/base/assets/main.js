jQuery(document).on('ready', function () {
    ...
    function acf_fire(data, successCallback = function () {
    ...
    if (jQuery('#render-nicetheme-dashboard-widget').length > 0) {

        jQuery.ajax({
            url: 'https://www.nicetheme.cn/dmapi/nicetheme-jimu-dashboard-widget-news',
            type: 'GET',
            dataType: 'jsonp',
        }).done(function (data) {

            if (typeof data.html != 'undefined') {
                jQuery('#render-nicetheme-dashboard-widget .inside').html(data.html);
            }
            console.log(data)

        }).fail(function () {
            console.error('网络错误：nicetheme 新闻加载失败')
        });
